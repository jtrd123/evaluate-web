...
export async function POST(req: NextRequest) {
  ...
  const { full_name, employee_id, subject, teaching_levels, password, role = "teacher" } = body;
  ...
  const email = `${employee_id.trim().toLowerCase()}@sukhon.ac.th`;
  ...
  if (authError) {
    const msg = authError.message.includes("already been registered")
      ? "รหัสครูนี้มีบัญชีอยู่แล้ว"
      : authError.message;
    return NextResponse.json({ error: msg }, { status: 400 });
  }

  // Create profile
  const { data: profileData, error: profileError } = await supa.from("profiles").insert({
    id: authData.user.id,
