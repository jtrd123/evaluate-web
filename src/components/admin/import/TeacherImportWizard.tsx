  ...
  async function handleImport() {
    ...
    for (let i = 0; i < rows.length; i += BATCH) {
      ...
      const res = await fetch("/api/admin/import", {
        ...
        body: JSON.stringify({ type: "teachers", rows: batch, skipExisting: mode === "new_only", academicYear }),
      });
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-3 bg-primary/5 border border-primary/20 rounded-2xl px-4 py-3">
          ...
          />
          <span className="text-xs text-primary/60">ครูที่ import จะถูกจัดกลุ่มตามปีนี้</span>
        </div>
